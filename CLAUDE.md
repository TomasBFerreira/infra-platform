...
- zigbee-iot: prod=189/190 (192.168.10.89/.90), dev=289/290 (192.168.20.89/.90), qa=389/390 (192.168.30.89/.90) — *Zigbee IoT power monitoring (Mosquitto + Zigbee2MQTT + Telegraf). Requires physical USB dongle on the target Proxmox node.*
...
- **External APIs (Anthropic, GitHub, …) also break unless ndots is lowered.** Same wildcard collision: `api.anthropic.com` has 2 dots, ndots=5 search-expands to `api.anthropic.com.databaes.net` and Cloudflare answers. Add `dnsConfig.options.ndots: "1"` to any deployment that calls a public API — **not** `"2"`: single-dot hostnames like `github.com` or `lichess.org` still get search-expanded at ndots=2. See `.claude/rules/kubernetes.md` "DNS gotchas" (corrected 2026-09-08 — this section previously said `"2"`). → `docs/network.md` → "Same wildcard, different shape".
...
- **HashiCorp's apt GPG key rotates.** `ansible/github-runner/github-runner_setup.yml` and `ansible/vault-ct/vault-ct_setup.yml` used to fetch it with a `creates:` guard, so a host's keyring got pinned forever at whatever key existed on first provision. HashiCorp rotated the key 2026-09-09; any host provisioned before that date will fail `apt-get update` against their repo with `NO_PUBKEY ...` until re-provisioned or the keyring is manually refreshed (`curl -fsSL https://apt.releases.hashicorp.com/gpg | gpg --batch --yes --no-tty --dearmor -o /usr/share/keyrings/hashicorp-archive-keyring.gpg`). Fixed going forward in both playbooks (no more `creates:` guard) 2026-09-10.
- **`secret/github-runner/gh_pat` (the classic PAT `register-runner.yml` uses to mint runner-registration tokens) expires and isn't auto-rotated** — same class of issue as the 2026-08-29 cascading-token-expiry incident. A 401 "Bad credentials" from `register-runner.yml` (even against a repo you know the PAT should have access to) means the PAT itself is dead, not a per-repo scope problem — verify with `curl -H "Authorization: Bearer $PAT" https://api.github.com/user`. GitHub gives no API to renew or create a classic PAT — a human has to generate a fresh one in the GitHub UI (Settings → Developer settings → Personal access tokens (classic) → Regenerate/new), then either `gh secret set GH_PAT` + run `sync-gh-pat-to-vault.yml`, or write it directly into the bootstrap vault at `secret/github-runner/gh_pat`.
- **The torrent CT (`terraform/torrent/main.tf`) was missing `onboot = true`** — every other LXC in this repo sets it, torrent didn't, so after betsy crashed 2026-09-14 the CT never came back and had to be started by hand. Fixed 2026-09-16. If a CT still needs a manual `pct start` after a host reboot, check its Terraform for a missing `onboot` flag before assuming it's a Proxmox-side issue.
