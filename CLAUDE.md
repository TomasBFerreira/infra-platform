...
5. **Blue/green pipeline** — every service must have a pipeline with all 6 standard jobs in order: `resolve-slots` → `terraform-staging` → `ansible-staging` → `flip-active` → `teardown-old-active` → `cleanup-on-failure`. Single-slot deployments are not allowed. Pipelines must use the shared env runner via `runs-on: [self-hosted, <env>]` (e.g. `[self-hosted, dev]`). Do not assume a personal dev machine or ad-hoc host. **Exceptions** (single-slot, `runs-on: [self-hosted, management]`):
   ...
   - `mail` — self-hosted Stalwart mail server. Stateful (mailboxes, outbound queue, DKIM keys); blue/green would lose queued/stored mail per flip or require copying the maildir + key material with downtime, for no benefit (one mail system serves the whole domain). **Runner caveat**: the current **dev validation** CT runs on `[self-hosted, dev]` (it lives on the dev subnet, like monitoring), not `[self-hosted, management]`; the future cluster-wide **production** mail singleton will move to the management subnet + `[self-hosted, management]` like pbs. See `/app/issues/self-service-accounts-2026-06-02.md`.
...
**Mail server (Stalwart) — single-slot (dev validation now; prod TBD):**
- mail: dev=204, IP 192.168.20.4 (benedict, vmbr20). Self-hosted SMTP/IMAP/JMAP for databaes.net. Single-slot per rule #5 exception. Outbound relays through the Brevo smarthost (deliverability from a residential/tunnel'd homelab); Stalwart owns mailboxes + (future) inbound. **No public Traefik route** (SMTP/IMAP aren't HTTP; can't sit behind forwardAuth) — reached internally by Authentik for submission, outbound via Brevo only; web admin is internal-only. App secrets in the dev env vault at `secret/mail/dev`; SSH key in bootstrap at `secret/ssh_keys/mail_worker`. qa/prod not yet deployed — prod will likely be a cluster-wide singleton on the management subnet (like pbs). Built for the self-service-accounts workstream; see `/app/issues/self-service-accounts-2026-06-02.md`.
