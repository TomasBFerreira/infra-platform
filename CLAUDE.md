...
>
> **Actually runs ALL prod k3s workloads (corrected 2026-09-08):** this VM is not just "streambox, media-stack, landing-page, status-api, cattle-cluster-agent" as the note above says — live `kubectl get pods -A` shows 112 pods: also every `ops-portal-*` service, wikijs, nextcloud, minecraft, databaes-investing, databaes-directus, otel-monitoring, signup-page. It's the sole prod k3s node — no HA, no second worker. See `/app/issues/wikijs-scale-to-zero-feasibility-2026-09-06.md` for the capacity investigation that found this.
>
> **Memory bumped 8192MB → 10240MB (2026-09-08)** to relieve node-capacity pressure (pod memory requests were at 89% of the 8GB node). Applied live via `qm set 121 -memory 10240` + a VM restart — **NOT via `worker-node-gpu_pipeline_self_hosted.yml`**. That pipeline's "pre-flight" step unconditionally runs `qm destroy 121 --purge` before every `terraform apply`, which would wipe every local-path PVC on this node (every service's Postgres, minecraft's world, nextcloud data — everything listed above). The pipeline is fresh-provision-only, not safe for reconciling a live singleton; treat it like the `pbs` single-slot exception in that respect. Terraform's `main.tf` was updated to `dedicated = 10240` to match, but re-running the pipeline against VMID 121 will still destroy it — don't, until that pipeline gets a real in-place-update path or a backup/restore step.
>
> **Known gotcha found during the above restart:** a pod requesting `nvidia.com/gpu` can hit `UnexpectedAdmissionError` ("no healthy devices present") if kubelet tries to (re)admit it before the `nvidia-device-plugin` DaemonSet finishes re-registering the GPU after a node reboot — and the ReplicaSet does **not** automatically replace a pod stuck in this state (confirmed: one sat for 35 days). After any reboot of VM 121, check for GPU-consuming pods stuck at `0/1` and `kubectl delete pod` them once `kubectl describe node worker-node-gpu-01 | grep nvidia.com/gpu` shows the device healthy again. Tracked as an open CMDB problem (`streambox`, `worker-node-gpu-01`).
