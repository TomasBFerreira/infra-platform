...
### Gotcha: AdGuard's real Tailscale hostname has no env prefix
The `<env>-adguard-lxc-<slot>` hostname `scripts/update-tailscale-split-dns.sh` (the automation that keeps tailnet-wide Split DNS for `databaes.net` pointed at the active AdGuard per env) used to match against **doesn't exist**. Every env's AdGuard registers under the identical hostname `adguard-lxc-<slot>` — Tailscale disambiguates same-name devices with its own registration-order `-1`/`-2`/`-3` DNS suffix, unrelated to env. The env only shows up in each device's ACL tags (`tag:dev`/`tag:qa`/`tag:prod`). A hostname-only match can never find the right device across three identically-named env instances — fixed 2026-09-09 to match on hostname **and** tag together. See [`/app/issues/tailscale-splitdns-hostname-mismatch-2026-09-09.md`](../../issues/tailscale-splitdns-hostname-mismatch-2026-09-09.md) for the full investigation. Symptom when this regresses: a `*-dev.databaes.net` / `*-qa.databaes.net` hostname resolves fine from something with a direct LAN route (an exit node, or a device already on that subnet) but not for an ordinary Tailscale peer relying on Split DNS.